    ...
    def update(self):
        ...
        if self.rpc:
            if presence.game.name: # Please file an issue if this happens to fail
                ...
                self.rpc.update(details = presence.game.name, large_image = presence.game.imageUri, large_text = presence.game.name, start = self.start, buttons = [{'label': 'Nintendo eShop', 'url': presence.game.shopUri},])
            else:
                self.currentGame = None
                self.rpc.clear()
        # Set GUI
